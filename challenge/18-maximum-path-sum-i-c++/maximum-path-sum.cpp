...
// int pyramid[PYRAMID_SIZE][PYRAMID_SIZE] = {
//   {3},
//   {7, 4},
//   {2, 4, 6},
//   {8, 5, 9, 3}
...
int getRightChild(int parentY, int parentX) {
  return pyramid[parentY + 1][parentX + 1];
}

int getLeftChild(int parentY, int parentX) {
  return pyramid[parentY + 1][parentX];
}

/**
 *  return X - index of larger child 
 */
int getGreatestChild(int parentY, int parentX) {
  // cout << "Currently at " << pyramid[parentY][parentX] << ". Comparing " << getLeftChild(parentY, parentX) << " with " << getRightChild(parentY, parentX) << "\n";
  if(getLeftChild(parentY, parentX) > getRightChild(parentY, parentX)) {
    // cout << "Looks like " << getLeftChild(parentY, parentX) << " is bigger.\n";
    return getLeftChild(parentY, parentX);
  }
  // cout << "Looks like " << getRightChild(parentY, parentX) << " is bigger.\n";
  return getRightChild(parentY, parentX);
}

void replaceRow(int row, int* newRow) {
  for(int col = 0; col < PYRAMID_SIZE; col++) {
    pyramid[row][col] = newRow[col];
  }
}

void filterAndReplaceRow(int rowNum){
  cout << "Filtering and replacing row: " << rowNum << "...\n";
  int newRow[PYRAMID_SIZE] = {};
  int rowToReplace = rowNum - 1;
  for(int col = 0; col < rowNum; col++){
    newRow[col] = pyramid[rowToReplace][col] + getGreatestChild(rowToReplace, col);
  }
  replaceRow(rowToReplace, newRow);
}

void printPyramid(int (&pyramid)[PYRAMID_SIZE][PYRAMID_SIZE]) {
  cout << "\n";
  for(int i = 0; i < PYRAMID_SIZE; i++) {
    printArray(pyramid[i]);
  }
  cout << "\n";
}
...
void filterAllRows() {
  for(int row = PYRAMID_SIZE - 1; row != 0; row--) {
    filterAndReplaceRow(row);
  }
}
...
int main() {
  ...
  filterAllRows();
  printPyramid(pyramid);
  cout << "Looks like the max was " << pyramid[0][0] << "\n";
}